...
public class UniqueShadow : MonoBehaviour {
	...
	void SetFocus(int idx) {
		...
		var shadowProjection = Matrix4x4.Ortho(-focus.radius, focus.radius, -focus.radius, focus.radius, 0f, focus.radius * 2f);
		...
		var isD3D = isD3D9 || SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.Direct3D11 || SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.Direct3D12;
#if UNITY_5_5_OR_NEWER
		var to = 0f;
		var db = SystemInfo.usesReversedZBuffer ? focus.depthBias : -focus.depthBias;
#else
		var to = isD3D9 ? 0.5f / (float)(int)shadowMapSize : 0f;
		var db = -focus.depthBias;
#endif
